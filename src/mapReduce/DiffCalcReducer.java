...
import org.apache.hadoop.mapreduce.Reducer;
 ...
 //takes the set of node, adsorpvals for each node from two directories and emits 
 //the maximum of all differences between corresponding pairs of adsorption values
public class DiffCalcReducer extends Reducer<Text,Text,IntWritable,DoubleWritable>
{
	public static final double D = 0.15;
	public void reduce(Text key, Iterable<Text> rankTexts, Context context)
		...
		for (Text rankText : rankTexts)
		{
			...
			{
				if (j==0) {
					ranks0.put(ranksValues[i], Double.parseDouble(ranksValues[i+1]));
				}
				if (j==1) {
					ranks1.put(ranksValues[i], Double.parseDouble(ranksValues[i+1]));
				}
			}
			j++;
		}
		...
		//emit largest diff for this node
